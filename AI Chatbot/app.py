...
def process_document(uploaded_file: Union[BytesIO, "streamlit.runtime.uploaded_file_manager.UploadedFile"]) -> List[Document]:
    ...
    Saves the uploaded file temporarily, loads the PDF, and splits the content
    into chunks using RecursiveCharacterTextSplitter.

    Args:
        uploaded_file: Streamlit uploaded file object (st.file_uploader)

    Returns:
        List of Document objects containing chunked text from the PDF
    """
    temp_file_path = None
    try:
        ...
        # Ensure temporary file is deleted safely
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                os.unlink(temp_file_path)
            except PermissionError:
                # Retry briefly if Windows locks the file
                time.sleep(0.1)
